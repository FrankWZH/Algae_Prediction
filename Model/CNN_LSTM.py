...
class CNN_LSTM:
    def __init__(self, input_unit: int, filters: int, kernel_size: int, activation: str, x_data, y_data, valid_data, epoch, batch_size):
    ...
    def set_optimizer(self):
        return tfa.optimizers.CyclicalLearningRate(initial_learning_rate=5e-07, maximal_learning_rate=5e-04,
    ...
    def build_model(self):
        self.model.add(Conv1D(filters=self.filters, kernel_size=self.kernel_size, activation=self.activation,
                       input_shape=self.input_shape))
        self.model.add(layers.LSTM(units=self.input_unit, return_sequences=True, activation=self.activation))
