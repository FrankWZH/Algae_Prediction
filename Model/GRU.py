...
class GRU:
    def __init__(self, input_unit: int, activation: str, x_data, y_data, valid_data, epoch, batch_size):
        ...
        self.callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)

    def set_optimizer(self):
        return tfa.optimizers.CyclicalLearningRate(initial_learning_rate=1e-06, maximal_learning_rate=1e-04,
    ...
    def build_model(self):
        self.model.add(layers.GRU(units=self.input_unit, return_sequences=True, activation=self.activation,
                                  input_shape=self.input_shape))
        self.model.add(Flatten())
        self.model.add(Dense(units=256, activation=self.activation))
